...
- **Quantized inference** — FP16, BF16, Q8_0, Q4_K_M and other GGUF quantization formats; fused scale×int dot-product kernels operating directly on quantized blocks
...
- **2026-04** — BF16 quantization support — GGUF tensor type 30 (BF16) fully supported across CPU and GPU backends. CPU kernels dispatch to BF16-specialized GEMV/GEMM paths, fused decode handles BF16 projections, CUDA backend converts BF16 to FP16 at load time. Added 15 unit tests covering dequantization, matmul, parallel execution, and fused decode
