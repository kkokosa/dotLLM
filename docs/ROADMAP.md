...
| 57 | **Gemma 4 architecture** :white_check_mark: | Google Gemma 4 model family (released 2026-04-02, Apache 2.0). New `Architecture.Gemma4` enum variant dispatched from `model_type=gemma4_text` / `gemma4` / `gemma4_unified` (and architectures[0]=`Gemma4ForCausalLM` / `Gemma4ForConditionalGeneration` / `Gemma4UnifiedForConditionalGeneration`). HF multimodal/unified checkpoints house the text tower under `text_config`; `HfConfigExtractor` hoists that sub-object exactly as for Gemma 3. Forward wiring reuses every Gemma 2/3 mechanism already shipped — GeGLU activation (gelu_pytorch_tanh), RMS pre-norm with per-layer (1+w) scaling, four RMSNorms per layer, per-head Q/K RMSNorm, interleaved local/global attention via the `layer_types` array (canonical 5×sliding + 1×full pattern), attention + final logit soft-capping (default `final_logit_softcapping=30.0`), and query-pre-attn scalar — all surfaced through the existing `ModelConfig.PerLayerSlidingWindow` / `AttnLogitSoftcap` / `FinalLogitSoftcap` / `QueryPreAttnScalar` fields. `ModelLoader.LoadFromSafetensors` dispatches `Architecture.Gemma4` through the dense `TransformerModel.LoadFromSafetensors` path. RoPE convention: NeoX (matches `rotate_half` in `modeling_gemma4.py`). Tied embeddings default true. Out of scope (future): split `rope_parameters.{full,sliding}_attention` rope_theta (Gemma 4 uses different theta per layer-type; the current loader reads top-level `rope_theta` which is still present as a fallback), `global_head_dim` / `num_global_key_value_heads` (alternative head shape on full-attention layers — null on the public 12B/31B), Gemma 4 26B-A4B MoE variant (`enable_moe_block` + `num_experts`, will wire through the existing MoE path), and multimodal vision/audio towers (text-only forward only). Verified: 5 HfConfigExtractor detection tests (text-only `Gemma4ForCausalLM`, multimodal `Gemma4ForConditionalGeneration` text_config hoist, unified `Gemma4UnifiedForConditionalGeneration`, omitted-tie default, generic-Gemma priority guard), and 2 synthetic-fixture forward-pass tests (all-mechanisms + public-12B-defaults). | Phase 1 |
