...
| 34c | **Microsoft.Extensions.AI `IChatClient`** :white_check_mark: | `DotLLM.Extensions.AI` package — `DotLLMChatClient : IChatClient` over `TextGenerator`. Native, in-process Microsoft Agent Framework / MEAI backend (`chatClient.CreateAIAgent(...)`): streaming, tool calling (`FunctionCallContent`), structured output, usage. AOT-clean, depends only on `Microsoft.Extensions.AI.Abstractions`. See `docs/EXTENSIONS_AI.md`. | 16, 42 |
