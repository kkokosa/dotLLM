...
//   Grid: ceil(total_blocks / 8) CUDA blocks × 256 threads = 8 warps per CUDA block.
// - Q4_K/Q5_K/Q6_K: one CUDA block (256 threads) per superblock, each thread writes one element.
//   Grid: total_superblocks CUDA blocks × 256 threads.
    ...
    const int total_blocks)
{
    ...
    int block_idx = global_tid / Q8_0_BLOCK_SIZE;
    int lane = global_tid % Q8_0_BLOCK_SIZE;
...
{
    int global_tid = blockIdx.x * blockDim.x + threadIdx.x;
    int block_idx = global_tid / Q4_0_BLOCK_SIZE;
    int lane = global_tid % Q4_0_BLOCK_SIZE;
    if (block_idx >= total_blocks) return;
    ...
    // lane 0..31 maps to output elements 0..31
...
{
    int global_tid = blockIdx.x * blockDim.x + threadIdx.x;
    int block_idx = global_tid / Q5_0_BLOCK_SIZE;
    int lane = global_tid % Q5_0_BLOCK_SIZE;
    if (block_idx >= total_blocks) return;
    ...
    const int total_superblocks)
{
    ...
    // pos 0..15 → low nibble of qs[pos], pos 16..31 → high nibble of qs[pos-16]
    // But the original layout writes: sub_out[2*j+0]=lo, sub_out[2*j+1]=hi for j=0..15
    // So: even pos → low nibble of qs[pos/2], odd pos → high nibble of qs[pos/2]
    int j = pos / 2;
    ...
    const int total_superblocks)
{
    ...
    // group 0: ql_half[l] & 0x0F | qh bits → elements 0..31
    // group 1: ql_half[l+32] & 0x0F | qh bits → elements 32..63
    // group 2: ql_half[l] >> 4 | qh bits → elements 64..95
    // group 3: ql_half[l+32] >> 4 | qh bits → elements 96..127
